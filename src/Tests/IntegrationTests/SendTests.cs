...
namespace Rebus.IntegrationTesting.Tests.IntegrationTests
{
    ...
    public class SendTests
    {
        ...
        public async Task SendsMessage()
        {
            ...
            var incomingCommand = (IncomingCommand) (await _bus.GetPendingMessages()).Select(m => m.Body).Single();
            Assert.That(incomingCommand.Value, Is.EqualTo("Hello World"));

            await _bus.ProcessPendingMessages();

            var outgoingCommand = (OutgoingCommand) (await _bus.GetMessages("OtherQueue")).Select(m => m.Body).Single();
